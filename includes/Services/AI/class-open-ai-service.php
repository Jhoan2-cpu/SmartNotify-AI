...
class OpenAIService implements AIServiceInterface {
    ...
    public function generate($prompt, array $options = []) {
        ...
        if ($response_code !== 200) {
            ...
            error_log('OpenAI API Error (HTTP ' . $response_code . '): ' . $error_message);

            if ($response_code === 401) {
                throw new \Exception('API Key inválida. Verifica tu clave de OpenAI.');
                ...
                throw new \Exception('Sin créditos. Recarga tu cuenta de OpenAI.');
            } else {
